...
import su.nextlevel.launcher.util.FileManager;
...
public class Main extends Application {
        ...
        try {
            ...
            if (!isNew.getValue()) {
                ...
                loginData = LoginData.instance(
                        SimpleTokenCodec.getInstance().fromCoded(
                                readFile(dataDirectory.getCanonicalPath() + File.separator + "token.txt",
    ...
    public void start(Stage primaryStage) throws Exception {
        ...
        if (manager.mustRunIntoSetup()) {
            ...
            stage.setScene(new Scene(FXMLLoader.load(Objects.requireNonNull(ConstantPool.CLASS_LOADER.getResource("setup.fxml"))), 328, 192));
            primaryStage = stage;
            primaryStage.show();
            ...
            if (isNewBool) {
                primaryStage.setScene(new Scene(FXMLLoader.load(Objects.requireNonNull(ConstantPool.CLASS_LOADER.getResource("login.fxml"))), 350, 438));
            } else {
                if (loginData == null) {
                    primaryStage.setScene(new Scene(FXMLLoader.load(Objects.requireNonNull(ConstantPool.CLASS_LOADER.getResource("login.fxml"))), 350, 438));
                } else {
                    primaryStage.setScene(new Scene(FXMLLoader.load(Objects.requireNonNull(ConstantPool.CLASS_LOADER.getResource("launcher.fxml"))), 951, 580));
                }
            }
        }
        primaryStage.getIcons().add(SwingFXUtils.toFXImage(ImageIO.read(Objects.requireNonNull(ConstantPool.CLASS_LOADER.getResource("logo.png"))), null));
